...
To disable the `Button` and change its text during processing, you might be tempted to use the `doOnNext()` to achieve this.
...
But if you try to execute a request this way, you will get an error indicating that the `submitButton` is not being modified on the FX thread. This is occuring because an IO thread (not the FX thread) is trying to modify the `Button`. You could move this `doOnNext()` operator before the `observeOn(Schedulers.io())` and that would address the issue. But there will be times where you must call a `doOnNext()` deep in an `Observable` chain that is already on another thread (such as updating a `ProgressBar`).
...
Did you know the `flatMap()` is actually a powerful concurrency tool? RxJava by default does not do parallelization, and as we have seen in this chapter `subscribeOn()` and `observeOn()` merely move emissions from one thread to another thread, not one thread to many threads. So effectively, there is no way to parallelize an `Observable`.

However, you can leverage `flatMap()` to create several Observables parallelizing emissions on different threads from a `Scheduler`.
...
Your output may look very different from what is above, and that is okay because nothing is deterministic when we do this sort of concurrency. Threads will be assigned at random. Since each `Observable` created by an emission inside a `flatMap()` can take its own thread from a given `Scheduler`, each resulting `Observable` will indepedently process emisssions on a separate thread _but only within the `flatMap()`_.

It is critical to note that the `flatMap()` can fire emissions from multiple Observables inside it, all of which may be running on different threads. To respect the `Observable` contract, it must make sure that emissions leaving the `flatMap()`and going to the `Subscriber` are serialized. If one thread/Observable is busy pushing items out of the `flatMap()`, the other threads/Observables will leave their emissions for that thread to take ownership of in a queue. This allows the benefit of concurrency without any blocking or synchronization of threads.


## Summary

In ths chapter we have learned the main selling point of Rxjava: concise and composable concurrency.You can compose Observables to change concurrency policies at any time with the `subscribeOn()` and `observabeOn()`. This makes applications adaptable, scalable, and evolvable over longer periods of time. You do not have to mess with synchronziers, semaphores, or any other low-level concurrency tools as RxJava takes care of these complexities for you.
