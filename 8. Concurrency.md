...
You can then put in a URL in the `TextField` (such as "https://api.github.com/users/thomasnield/starred") and then click the "Submit" `Button` to process it. You will notice the UI stays interactive and after a few seconds it will put the response in the `TextArea` (Figure 8.5).
