...
With both concatentation and merging, you can combine as many Observables as you want. But thes two operators work with Observables emitting the same type `T`. There are ways to combine emissions of different types which we will see next.
...
If you want to get extra crazy, you can easily modify this operation so that the combined `ObservableList` only contains *distinct* items from both ObservableLists. Simply add a `flatMap()` before the `Subscriber` that intercepts the `ArrayList`, turns it into an `Observable`, distincts it, and collects it back into a `List`. Notice when you run it, the duplicate "Dallas" emission is held back.
...
Do not worry if you find the code above odd and hard to understand right now. It is a creative solution, and you can feel free to move on and study it again later as you get more comfortable with Rx.
