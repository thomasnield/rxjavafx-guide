...
When working with Observables built off UI events, sometimes an error can occur and will be communicated up the chain via `onError()`. In production, you should always have the `Subscriber` handle an `onError()` so the error does not just quietly disappear. But when you are dealing with UI input events, there is likely one other error handling issue to consider.
...
There is one problem here. The `Observable` is now dead. It called `onError()` and closed the stream, assuming nothing could be done to recover from it. You will find the `Button` is no longer sending emissions, and you can fix this by adding the `retry()` operator right before the `Subscriber`. When its `onError()` is called, it will stop the error and resubscribe again, giving another chance in hopes an error does not happen again.
