...
If you type in a non-numeric input, it will resubscribe and start all over. The `scan()` operator will send another initial emission of `0` and result in everything being reset. You also have the option of moving the `retry()` before the `scan()` operation, and that would maintain the current rolling total rather than canceling it and starting over at 0. 
