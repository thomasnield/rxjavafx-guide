...
For the next few examples, let's just focus on the `Observable` chain. Notice that the `JavaFxObservable.valuesOf()` (or `toObservable()` for Kotlin) does not push the initial null value, because RxJava 2 does not emit null values. If you want to emit an `Optional<T>`to handlue nullability, use the `JavaFxObservable.nullableValuesOf()` factory as shown below:
