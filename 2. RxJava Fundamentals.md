...
Some operators will aggregate the emissions in some form (in a class [MapReduce](https://en.wikipedia.org/wiki/MapReduce) fashion), and then push that aggregation as a single emission to the `Observer`. Obviously, this requires the `onComplete()` to be called so that the aggregation can be finalized and pushed to the `Observer`.
...
The `count()` actually returns a `Single`, which is a specialized `Observable` type that only emits one item. The `Single` does not have an `onNext` or `onComplete`, but rather an `onSuccess` event which passes the single item. If you ever need to turn a `Single` back into an `Observable` (so it works with other operators), just call its `toObservable()` method.
