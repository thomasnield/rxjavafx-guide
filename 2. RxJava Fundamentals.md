...
In RxJava, you can use hundreds of operators to transform emissions and create new Observables with those transformations. For instance, you can create an `Observable<Integer>` off an `Observable<String>` by using the `map()` operator to map each String's length. 
...
The `source` Observable pushes each `String` to the `map()` operator `Observable` where it is transformed to its `length()`. That length is then pushed from the `map()` operator to the `Subscriber` where it is printed. 